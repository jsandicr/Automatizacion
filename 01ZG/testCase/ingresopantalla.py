    ...
    def ingresopantalla(self, conditions, Configuracion):
        ...
        try:
            ...
            action \
                ...
                .release()
            action.perform()
            Zonas = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='_tablas_zonas']")))
            action = ActionChains(self.driver)
            action \
                .move_to_element(Zonas) \
                .pause(0) \
                .release()
            action.perform()
            ZonasGeo = self.wait.until(conditions.visibility((By.XPATH, "//*[@id='zonas']")))
            action = ActionChains(self.driver)
            action \
                .click(ZonasGeo) \
                .pause(0) \
            ...
            Log().info(" Abre la pantalla de Zonas Geográficas")
        ...
        try:
            ...
            self.assertEqual("ZONAS GEOGRÁFICAS", Pantalla, "La pantalla ejecutada es correcta")
            timeStrmap = time.strftime('%Y%m%d_%H_%M_%S')
            img_name = os.path.join(img_path, "%s.png" % str(timeStrmap))
            Log().info(message=" Captura: %s  screen：%s" % (img_path, os.path.split(img_name)[1]))
            self.driver.get_screenshot_as_file(img_name)
            Log().info(" La pantalla ejecutada es Zonas Geográficas.")
        ...
        try:
            Registrorepetido = self.driver.find_element(By.XPATH, "//span[text()='12345']")
